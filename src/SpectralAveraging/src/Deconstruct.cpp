...
namespace SpectralAveraging
{
      ...
      MonolithicDeconstruct(const std::shared_ptr<CSpectralSampleData> & sampdat)
    {
        ...
        FenestrationCommon::CSeries Ts =
          sampdat->properties(FenestrationCommon::Property::T, FenestrationCommon::Side::Front);
        FenestrationCommon::CSeries Rfs =
          sampdat->properties(FenestrationCommon::Property::R, FenestrationCommon::Side::Front);
        FenestrationCommon::CSeries Rbs =
          sampdat->properties(FenestrationCommon::Property::R, FenestrationCommon::Side::Back);
                                             ...
                                             const std::shared_ptr<CSpectralSampleData> & subdat)
    {
        ...
        FenestrationCommon::CSeries rfc =
          (Rfc - sprop.surface.rfs) / tau2 / (sprop.surface.rfs * (Rfc - 2.) + 1.);

        FenestrationCommon::CSeries tc =
          Tc * (1 - sprop.surface.rfs * rfc * tau2) / sprop.surface.ts / sprop.taus;
                                 ...
                                 const std::shared_ptr<CSpectralSampleData> lamdat)
    {
        ...
        FenestrationCommon::CSeries Tl =
          sampdat->properties(FenestrationCommon::Property::T, FenestrationCommon::Side::Front);
        FenestrationCommon::CSeries Rfl =
          sampdat->properties(FenestrationCommon::Property::R, FenestrationCommon::Side::Front);
        FenestrationCommon::CSeries Rbl =
          sampdat->properties(FenestrationCommon::Property::R, FenestrationCommon::Side::Back);

        FenestrationCommon::CSeries Rfprim = (Rfl - sprop1.surface.rfs)
                                             / (sprop1.taus * sprop1.taus)
                                             / (sprop1.surface.ts * sprop1.surface.ts
                                                + sprop1.surface.rfs * (Rfl - sprop1.surface.rfs));
        FenestrationCommon::CSeries Tprim =
          Tl * (1 - sprop1.taus * sprop1.taus * Rfprim * sprop1.surface.rfs) / sprop1.surface.ts
          / sprop1.taus;
        ...
        FenestrationCommon::CSeries rgcdenom = taupvb * taupvb * sprop2.taus * sprop2.taus
                                               * ((Rbprim - sprop2.surface.rfs) * sprop2.surface.rfs
                                                  + sprop2.surface.ts * sprop2.surface.ts);
